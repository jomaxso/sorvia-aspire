...
//     private readonly Dictionary<string, PublishedComposeServiceSnapshot> _publishedComposeServices = [with(StringComparer.OrdinalIgnoreCase)];
...
//     /// </summary>
//     internal static async Task<string?> ResolveServerUrlAsync(DokployEnvironmentResource environment, CancellationToken ct)
//     {
//         if (environment.ServerUrlParameter is not null)
//             return await environment.ServerUrlParameter.GetValueAsync(ct).ConfigureAwait(false) ?? DefaultServerUrl;
...
//     /// </summary>
//     internal static async Task<string?> ResolveApiKeyAsync(DokployEnvironmentResource environment, CancellationToken ct)
//     {
//         if (environment.ApiKeyParameter is null)
//             throw new InvalidOperationException($"Dokploy API key is required but not configured. Please provide an API key via the '{nameof(environment.ApiKeyParameter)}' property or ensure the parameter value is set during deployment.");

//         return await environment.ApiKeyParameter.GetValueAsync(ct).ConfigureAwait(false);
//     }

//     /// <summary>
//     /// Resolves the Dokploy project name from the generated parameter or the resource name.
//     /// </summary>
//     internal static async Task<string> ResolveProjectNameAsync(DokployEnvironmentResource environment, CancellationToken ct)
//     {
//         if (environment.ProjectNameParameter is not null)
//         {
//             var value = await environment.ProjectNameParameter.GetValueAsync(ct).ConfigureAwait(false);
...
//         throw new InvalidOperationException($"Dokploy project name is required but not configured. Please provide a project name via the '{nameof(environment.ProjectNameParameter)}' property or ensure the parameter value is set during deployment.");
//     }

//     /// <summary>
//     /// Resolves the Dokploy environment name used for deployments within the target project.
//     /// </summary>
//     internal static async Task<string> ResolveDeploymentEnvironmentNameAsync(DokployEnvironmentResource environment, CancellationToken ct)
//     {
//         string? value = null;

//         if (environment.DeploymentEnvironmentNameParameter is not null)
//         {
//             value = await environment.DeploymentEnvironmentNameParameter.GetValueAsync(ct).ConfigureAwait(false);
//         }
...
//     /// </summary>
//     public async Task DeployToDokployAsync(PipelineStepContext context, DokployEnvironmentResource environment)
//     {
...
//         var applicationName = await ResolveProjectNameAsync(environment, ct).ConfigureAwait(false);
//         var deploymentEnvironmentName = await ResolveDeploymentEnvironmentNameAsync(environment, ct).ConfigureAwait(false);
...
//         {
//             var serverUrl = await ResolveServerUrlAsync(environment, ct).ConfigureAwait(false);
//             var apiKey = await ResolveApiKeyAsync(environment, ct).ConfigureAwait(false);
